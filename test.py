...
parser.add_argument('gpuId', type=list, default=0 help='GPU ID used for testing')
parser.add_argument('checkpointV', type=str, 
                    default='modelCheckpoints/fusionNet_EmoWild_Valence' help='Valence prediction model')
parser.add_argument('checkpointA', type=str, 
                    default='modelCheckpoints/fusionNet_EmoWild_Arousal' help='Arousal prediction model')
...
# Build the model
fusionNet = FusionNetwork().to(gpuId)
fusionNet = torch.nn.DataParallel(fusionNet, device_ids=gpuId)
...
for root, _, fnames in sorted(os.walk(dir)):
    for imgname in fnames:
        ...
        sample = utils.move_to_cuda(sample, device=0) if use_cuda else sample
        ...
        fusionNet.load_state_dict(torch.load(checkpointV), strict=False)
        ...
        fusionNet.load_state_dict(torch.load(checkpointA), strict=False)
